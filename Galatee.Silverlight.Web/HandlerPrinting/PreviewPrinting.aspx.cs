...
{
    public partial class PreviewPrinting : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!Page.IsPostBack)
                {
                    //string ControlName = Convert.ToString(Request.QueryString["ControlName"]);
                    //module = Convert.ToString(Request.QueryString["Module"]);
                    //string matricule = Convert.ToString(Request.QueryString["Matricule"]);
                    //rdlc = ControlName.Substring(2);
                    ////  creation du use control relatif au parametre d url 
                    //string virtualPath = @"~/" + string.Format(@"{0}/{1}/{2}", namesp, module, ControlName);
                    //Web.Utility.CurrentModule = module;
                    //if(control == null)
                    //    control = (Previews.UcGeneric)LoadControl(virtualPath + ".ascx");
                    //if (control != null)
                    //{
                    //    control.Matricule = matricule;
                    //    Ucontainer.Controls.Add(control);
                    //}
